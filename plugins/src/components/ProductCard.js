...
export const ProductCard = ({ product }) => {
  ...
  return (
    <Link
      ...
      className="group relative rounded-xl border border-border bg-card overflow-hidden block transition-all duration-200 hover:border-neon/60 hover:neon-border hover:-rotate-1"
    ...
    >
      <div className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#0C0B09]/70 via-transparent to-transparent opacity-70" />
        {product.category && (
          <span className="absolute left-3 top-3 rounded-sm bg-[#0C0B09]/80 border border-neon/40 px-2 py-0.5 text-[10px] font-mono uppercase tracking-widest text-neon">
            {product.category}
          </span>
        )}
        <div className="absolute inset-x-3 bottom-3 flex gap-2 opacity-0 translate-y-2 transition-[opacity,transform] duration-200 group-hover:opacity-100 group-hover:translate-y-0">
          <Button
            size="sm"
            className="flex-1 rounded-md bg-neon text-[#0C0B09] hover:bg-neon/90 shadow-[0_0_22px_rgba(244,212,0,0.3)] font-mono uppercase tracking-widest text-xs"
            onClick={quickAdd}
